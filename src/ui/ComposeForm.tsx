...
import { receiptFromText, type StyleChoice } from "../core/text";
...
export type { StyleChoice } from "../core/text";
...
export const resolveReceipt = (values: ComposeValues): Receipt =>
  receiptFromText(values.body, { title: values.title, style: values.style });
...
export function ComposeForm({ initial, record, enableDrafts, onDone }: Props) {
  ...
  async function save() {
    ...
    if (record && record.kind !== "history") {
      // The text is what was edited, so stored items (with their dates) no longer apply.
      await store.save({ ...record, style: receipt.style, title: receipt.title, body: receipt.body, items: undefined });
    } else {
      await store.create({
        kind: "draft",
        style: receipt.style,
  ...
  return (
    ...
    >
      <Form.TextArea
        id="body"
        title="Text"
        placeholder="A list, one item per line (“call dentist tomorrow 3pm”), or a note"
        enableMarkdown
        autoFocus
        value={values.body}
        error={bodyError}
      ...
      >
        ...
        <Form.Dropdown.Item value="checklist" title="Checklist" icon={Icon.BulletPoints} />
        <Form.Dropdown.Item value="ticket" title="Ticket" icon={Icon.Receipt} />
        <Form.Dropdown.Item value="plain" title="Plain" icon={Icon.Text} />
      </Form.Dropdown>
      <Form.Description
        title="Formatting"
        text="Checklist: one item per line; dates like “friday 3pm” group the list by day. Memo: **bold**, __underline__, ==highlight==, # headings, - lists, | tables |, and pictures on a line of their own: ![caption](~/Pictures/photo.jpg)."
      />
    </Form>
  );
}
